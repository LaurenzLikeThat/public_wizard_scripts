...
async def patch(client:Client) -> List[tuple[int, bytes]]:
    ...
    async def show_guild_cooldown_patch():
        write_bytes = b"\xE9\xCE\x00\x00\x00\x90"
        pattern = rb"\x0F\x84\xCD\x00\x00\x00\x0F\x28\x44\x24."
        address_oldbytes.append(await readbytes_writebytes(pattern, write_bytes))

    await show_guild_cooldown_patch()
